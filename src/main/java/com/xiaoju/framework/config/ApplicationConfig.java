...
public class ApplicationConfig {
     ...
     * 
     * @see org.apache.tomcat.websocket.pojo.PojoEndpointServer#onOpen(javax.websocket.Session,
     *      javax.websocket.EndpointConfig) 这个函数当中的 pojo =
     *      sec.getConfigurator().getEndpointInstance(sec.getEndpointClass());
     *      会生成一个默认初始化处理器org.apache.tomcat.websocket.server.DefaultServerEndpointConfigurator
     *      然后在函数public <T> T getEndpointInstance(Class<T> clazz)下调用return
     *      clazz.getConstructor().newInstance(); 完成基于反射的实例构造
     */
    @Autowired
