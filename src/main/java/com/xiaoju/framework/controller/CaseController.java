...
public class CaseController {
    ...
    public Response<?> getCaseList(@RequestParam @NotNull(message = "渠道为空") Integer channel,
            ...
            @RequestParam @NotNull(message = "文件夹未选中") String bizId, @RequestParam(required = false) String title,
            @RequestParam(required = false) String creator, @RequestParam(required = false) String requirementId,
            @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime,
            @RequestParam(defaultValue = "1") Integer pageNum, @RequestParam(defaultValue = "10") Integer pageSize) {
        return Response.success(caseService.getCaseList(new CaseQueryReq(0, title, creator, requirementId, beginTime,
    ...
    public Response<?> createOrCopyCase(@RequestBody CaseCreateReq request) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.error("[Case Create]Create or duplicate test case failed. params={}, e={} ", request.toString(),
                    e.getMessage());
    ...
    public Response<?> editCase(@RequestBody CaseEditReq request) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.error("[Case Update]Update test case failed. params={} e={} ", request.toString(), e.getMessage());
            return Response.build(StatusCode.SERVER_BUSY_ERROR);
        }
    }
    ...
    public Response<?> deleteCase(@RequestBody CaseDeleteReq request) {
        ...
        try {
            ...
            Log.error("[Case Delete]Delete test case failed. params={} e={} ", request.toString(), e.getMessage());
            e.printStackTrace();
            return Response.build(StatusCode.SERVER_BUSY_ERROR);
        }
    }
    ...
    public Response<?> getCaseDetail(@RequestParam @NotNull(message = "用例id为空") Long caseId) {
        try {
            ...
            e.printStackTrace();
            Log.error("[Case detail]View detail of test case failed. params={}, e={} ", caseId, e.getMessage());
            return Response.build(StatusCode.SERVER_BUSY_ERROR);
        }
    }
    ...
    public Response<?> updateWsCase(@RequestBody WsSaveReq req) {
        try {
            ...
            e.printStackTrace();
            Log.error("[Case Update]Update test case failed. params={} e={} ", req.toString(), e.getMessage());
            return Response.build(StatusCode.SERVER_BUSY_ERROR);
        }
    }
