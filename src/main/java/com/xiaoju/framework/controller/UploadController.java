...
public class UploadController {
    ...
    public Response<Long> importXmind(@RequestParam MultipartFile file, String creator, String bizId,
        ...
        FileImportReq req = new FileImportReq(file, creator, bizId, productLineId, title, description, channel,
                requirementId);
        req.validate();
        try {
            ...
            Log.error("[导入x-mind出错] 传参req={},错误原因={}", req.toString(), e.getMessage());
            e.printStackTrace();
    ...
    /**
     * 根据caseId导出用例 response 文件在http响应中输出
     *
     * @param id 用例id
     */
    @GetMapping(value = "/export")
    public void exportXmind(@RequestParam @NotNull(message = "用例id为空") Long id, HttpServletRequest request,
        ...
        try {
            ...
            Log.error("[导出x-mind错误] caseId={}, 错误原因={}", id, e.getMessage());
            e.printStackTrace();
