...
public class WebSocketFilter extends FilterRegistrationBean<WsFilter> {
    ...
    class WsAuthFilter extends WsFilter {
        final Logger Log = LoggerFactory.getLogger(WsAuthFilter.class);
        ...
        private void authenticateByCookie(HttpServletRequest req) {
            ...
            List<String> authorityContent = roleAuthority
                    .get(StringUtils.isEmpty(authorityName) ? "ROLE_USER" : authorityName);
            ...
            for (String auth : authorityContent) {
                ...
                if (bRet) {
                    Log.info("权限认证成功, auth:" + auth + ", path: " + path);
                    return;
                }
            }
            Log.info("权限认证失败, request path: " + path + "，username: " + username);
