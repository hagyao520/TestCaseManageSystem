...
public class Client {
    ...
    public void close() {
        Log.info(Thread.currentThread().getName() + ": client " + this.getClientName() + " 准备退出。"
                + this.session.getId());
    ...
    public void sendMessage(CaseMessageType type, String msg) {
        if (!type.equals(CaseMessageType.PING)) {
            Log.info(Thread.currentThread().getName() + ": 准备向 " + this.getClientName() + " 发送消息：" + msg);
        }
        synchronized (messagesToSend) {
            if (!isClosing) {
                ...
                if (isSendingMessage) {
                    Log.warn(Thread.currentThread().getName() + ": 消息正在发送中，先缓存住. msg： " + msg);
                    // 检查缓存消息大小或数量是否异常
                    if (messagesToSend.size() >= 1000 || messagesToSendLength >= 1000000) {
                        isClosing = true;

                        // 丢掉异常的消息，关闭session
                        CloseReason cr = new CloseReason(CloseReason.CloseCodes.VIOLATED_POLICY,
                        ...
                        try {
                            ...
                            Log.error(Thread.currentThread().getName() + ": 接收的信息异常，关闭session.", e);
                        }
    ...
    private void internalSendMessageAsync(String msg) {
        try {
            if (msg instanceof String) {
                ...
                Log.info(Thread.currentThread().getName() + ": 消息中包含关闭信息，关闭session. ");
                session.close();
            }
        } catch (IllegalStateException | IOException e) {
            Log.error(Thread.currentThread().getName() + ": Client发送消息到session过程中，session被关闭。", e);
        }
    }
        ...
        public void onResult(SendResult result) {
            if (!result.isOK()) {
                ...
                try {
                    ...
                    // 忽略
                    Log.error(Thread.currentThread().getName() + ": client " + session.getId() + " 中session关闭异常.");
                }
            }
            synchronized (messagesToSend) {
                if (!messagesToSend.isEmpty()) {
