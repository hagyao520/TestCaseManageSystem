...
public class RecordRoom extends Room {
    private static final Logger Log = LoggerFactory.getLogger(RecordRoom.class);
    ...
    protected void internalRemovePlayer(Player p) {
        ...
        if (players.size() == 0) {
            if (testCaseContent != null) {
                ...
                List<String> names = Arrays.stream(executors.split(COMMA)).filter(e -> !StringUtils.isEmpty(e))
                ...
                Log.info(Thread.currentThread().getName() + ": 数据库用例记录更新。");
            }
            Log.info(Thread.currentThread().getName() + ": 最后一名用户 " + p.getClient().getClientName() + " 离开，关闭。");
        }
    ...
    public void mergeRecoed(Long recordId, String caseContentStr) {
        ...
        if (dto == null) {
            ...
            assert false;
            Log.error(Thread.currentThread().getName() + ": 当前用例执行者初次打开任务");
        }
        ...
        if (StringUtils.isEmpty(recordContent)) {
            // 其实当前任务还没有任何执行记录
            Log.info(Thread.currentThread().getName() + ": first create record.");
