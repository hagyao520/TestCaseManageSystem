...
public abstract class Room {
    ...
    public static boolean isBufferMessages() {
        return BUFFER_MESSAGES;
    }

    public Timer getMessageBroadcastTimer() {
        return messageBroadcastTimer;
    }
    ...
    public TimerTask getActiveBroadcastTimerTask() {
        return activeBroadcastTimerTask;
    }

    public void setActiveBroadcastTimerTask(TimerTask activeBroadcastTimerTask) {
        this.activeBroadcastTimerTask = activeBroadcastTimerTask;
    }
    ...
    public Map<Session, Client> getCs() {
        return cs;
    }

    public static TestCaseMapper getCaseMapper() {
        return caseMapper;
    }

    public static void setCaseMapper(TestCaseMapper caseMapper) {
    ...
    public static CaseBackupService getCaseBackupService() {
        return caseBackupService;
    }

    public static void setCaseBackupService(CaseBackupService caseBackupService) {
    ...
    public List<Runnable> getCachedRunnables() {
        return cachedRunnables;
    }

    public void setCachedRunnables(List<Runnable> cachedRunnables) {
        this.cachedRunnables = cachedRunnables;
    }
    ...
    public Room(Long id) {
        ...
        if (StringUtils.isEmpty(res)) {
            Log.error(Thread.currentThread().getName() + ": 用例内容为空");
        }
    }
    ...
    public Player createAndAddPlayer(Client client) {
        ...
        Log.info(Thread.currentThread().getName() + ": 有新的用户加入。session id: " + client.getSession().getId());
        ...
        if (activeBroadcastTimerTask == null) {
            ...
            messageBroadcastTimer.schedule(activeBroadcastTimerTask, TIMER_DELAY, TIMER_DELAY);
        }
    ...
    protected void internalRemovePlayer(Player p) {
        ...
        Log.info(Thread.currentThread().getName() + ": 有用户 " + p.getClient().getClientName() + " 离开 session id:"
                + p.getClient().getSession().getId());
    ...
    private void internalHandleMessage(Player p, String msg, long msgId) {
    ...
    private void broadcastMessage(String msg) {
        if (!BUFFER_MESSAGES) {
            ...
            testCaseContent = ((JSONObject) request.get("case")).toJSONString().replace("\"base\":" + currentVersion,
            ...
            for (Player p : players) {
                if (sendSessionId.equals(p.getClient().getSession().getId())) { // ack消息
                    ...
                    String msgNotify = patch.toJSONString().replace("[[{",
                            "[[{\"op\":\"replace\",\"path\":\"/base\",\"value\":" + (currentVersion + 1) + "},{");
    ...
    private void broadcastTimerTick() {
        ...
        for (Player p : players) {
            ...
            // Log.info("当前的消息池消息数量为：" + caseMessages.size());
            if (caseMessages.size() > 0) {
                for (int i = 0; i < caseMessages.size(); i++) {
                    ...
                    if (i > 0) {
                        sb.append("|");
                        Log.error(Thread.currentThread().getName() + ": client: " + p.getClient().getClientName()
                                + " 此处可能会有问题，待处理 by肖锋. sb: " + sb);
                    }
    ...
    public static final class Player {
        ...
        public void removeFromRoom() {
            if (room != null) {
                Log.info("当前离开用户 " + this.getClient().getClientName() + " 的使用时长是："
                        + String.valueOf(System.currentTimeMillis() - this.enterTimeStamp));
        ...
        public void sendRoomMessageSync(CaseMessageType type, String content) {
            ...
            if (content.equals(CaseWsMessages.PING.getMsg())) {
                this.pingCount ++;
                if (!isPingNormal()) {
                    Log.error("服务端ping客户端3次失败，当前用户连接有问题：" + this.getClient().getClientName());
