...
public class WebSocket {
    ...
    // * 依赖
    // * @see
    // ApplicationConfig#setWebsocketService(com.xiaoju.framework.service.RecordService,
    // com.xiaoju.framework.mapper.TestCaseMapper)
    // */
    ...
    public String toString() {
        return String.format("[Websocket Info][%s]caseId=%s, sessionId=%s, recordId=%s, isCoreCase=%s",
                recordId == null || CaseWsMessages.UNDEFINED.getMsg().equals(recordId) ? "测试用例" : "执行任务", caseId,
                session.getId(), recordId, isCore);
    }
        ...
        Thread t = new Thread(() -> {
            while(true) {
                ...
                try {
                    ...
                    Log.error("ping thread sleep error.", e);
                }
            }
        });
        t.setDaemon(true);
            ...
            if (rooms.get(id) == null) {
                synchronized (roomLock) {
                    if (rooms.get(id) == null) {
                        ...
                        Log.info(Thread.currentThread().getName() + ": 新建Room成功，caseid=" + BitBaseUtil.getLow32(id)
    ...
    @OnOpen
    public void onOpen(@PathParam(value = "caseId") String caseId, @PathParam(value = "recordId") String recordId,
            @PathParam(value = "isCore") String isCore, @PathParam(value = "user") String user, Session session)
            throws IOException {
        ...
        Log.info(Thread.currentThread().getName() + ": [websocket-onOpen 开启新的session][{}]", toString());
            ...
            public void run() {
                try {
                    try {
                        ...
                        Log.info(Thread.currentThread().getName() + ": player " + client.getClientName() + " 加入: "
                                + player);
                    ...
                    Log.error(Thread.currentThread().getName() + ": Unexcepted exception. e: " + e.getMessage());
                }
            }
        });
    }
    ...
    public void onClose(@PathParam(value = "caseId") String caseId, @PathParam(value = "recordId") String recordId) {
        ...
        if (room.players.size() == 1) {
            synchronized (roomLock) {
                ...
                Log.info(Thread.currentThread().getName() + ": [websocket-onClose 关闭当前Room成功]当前sessionid:"
                        + room.players.get(0).getClient().getSession().getId());
            }
        }
        room.invokeAndWait(new Runnable() {
            @Override
                ...
                try {
                    if (player != null) {
                        ...
                        if (player.getRoom().getLock()
                                && player.getRoom().getLocker().equals(player.getClient().getSession().getId())) {
                    ...
                    Log.error(Thread.currentThread().getName() + ": 异常 " + e.toString(), e);
                }
            }
        });
    }
                        ...
                        switch (messageType) {
                            case '0': // 处理ping/pong消息
                                ...
                                if (messageContent.equals(CaseWsMessages.PING.getMsg())) {
                                    ...
                                    Log.error(Thread.currentThread().getName() + "ping pong 信息有误。消息是：" + message);
                                }
                                break;

                            case '1': // 处理编辑消息
                                Log.info(Thread.currentThread().getName() + ": 收到消息... onMessage: " + message.trim());
                                if (player != null) {
                                    // todo：此处分隔符待优化
                                    Log.info(Thread.currentThread().getName() + ": 消息内部处理中...");
                            ...
                            case '2': // 处理控制消息
                                Log.info(Thread.currentThread().getName() + ": 收到控制消息... onMessage: " + message.trim());
                                ...
                                break;
                            default:
                                Log.error(Thread.currentThread().getName() + ": 这个消息类型不符合预期：" + message.trim());
                                break;
                        }
                    } catch (RuntimeException e) {
                        // 客户端发送的无效信息
                        // TODO: 可能要关闭连接
                        Log.warn(Thread.currentThread().getName() + ": 运行异常: " + e.getMessage(), e);
    ...
    public void onError(Session session, Throwable e) {
        Log.info(Thread.currentThread().getName() + ": [websocket-onError 会话出现异常]当前session={}, 原因={}", session.getId(),
                e.getMessage());
        int count = 0;
        Throwable root = e;
        ...
        if (root instanceof EOFException) {
            ...
            Log.error(Thread.currentThread().getName() + ": [websocket-onError 会话出现异常]" + e.toString(), e);
        }
    }
}
