...
public class CaseServiceImpl implements CaseService {
    ...
    public PageModule<CaseListResp> getCaseList(CaseQueryReq request) {
        ...
        // select * from test_case where case_id in (request.getCaseIds()) [and ...any
        // other condition];
        List<TestCase> caseList = caseMapper.search(request.getCaseType(), caseIds, request.getTitle(),
                request.getCreator(), request.getRequirementId(), beginTime, endTime, request.getChannel(),
                request.getLineId());

        List<RecordNumDto> recordNumDtos = recordMapper.getRecordNumByCaseIds(caseIds);
        Map<Long, Integer> recordMap = recordNumDtos.stream()
                .collect(Collectors.toMap(RecordNumDto::getCaseId, RecordNumDto::getRecordNum));
        ...
        for (TestCase testCase : caseList) {
            ...
            res.add(buildListResp(testCase, recordMap.get(testCase.getId()), caseNum));
        }
    ...
    public DirTreeResp deleteCase(Long caseId) {
        ...
        DirNodeDto tree = dirService.getDirTree(testCase.getProductLineId(), testCase.getChannel());
        updateDFS(packageTree(tree), String.valueOf(caseId), new HashSet<>(),
                new HashSet<>(convertToList(testCase.getBizId())));
        updateBiz(testCase, tree);
    ...
    public CaseConditionResp getCountByCondition(CaseConditionReq req) {
        ...
        // 没有筛选，返回caseNum为null
        caseNum = (req.getPriority().size() == 0 && req.getResource().size() == 0) ? null
                : TreeUtil.getCaseNum(caseRoot, tags);
    ...
    public void wsSave(WsSaveReq req) {
        ...
        // if (editors.size() < 1) {
        // throw new CaseServerException("用例ws链接已经断开，当前保存可能丢失，请刷新页面重建ws链接。",
        // StatusCode.WS_UNKNOWN_ERROR);
        // }
        ...
        if (!StringUtils.isEmpty(req.getRecordId())) {
            ...
            // 看看是不是有重合的执行人
            List<String> names = Arrays.stream(dto.getExecutors().split(COMMA)).filter(e -> !StringUtils.isEmpty(e))
                    .collect(Collectors.toList());
     ...
     * @see #getCaseList
     */
    private CaseListResp buildListResp(TestCase testCase, Integer recordNum, Integer totalNum) {
        CaseListResp resp = new CaseListResp();
        BeanUtils.copyProperties(testCase, resp);
        resp.setRecordNum(recordNum == null ? 0 : recordNum);
        resp.setTotalNum(totalNum == null ? 0 : totalNum);
        return resp;
    }
    ...
    private CaseDetailResp buildDetailResp(TestCase testCase) {
        ...
        BeanUtils.copyProperties(testCase, resp);
        resp.setBiz(getBizFlatList(testCase.getProductLineId(),
                Arrays.asList(testCase.getBizId().split(SystemConstant.COMMA)), testCase.getChannel()).stream()
                        .filter(BizListResp::isSelect).collect(Collectors.toList()));
        resp.setProductLineId(testCase.getProductLineId());
        return resp;
    }
