...
public class RecordServiceImpl implements RecordService {
    ...
    public RecordGeneralInfoResp getGeneralInfo(Long recordId) {
        ...
        JSONObject merged = getData(new MergeCaseDto(testCase.getId(), record.getChooseContent(),
                record.getCaseContent(), record.getEnv(), 0L));
        // 开始构建响应体
    ...
    public Long addRecord(RecordAddReq req) {
        JSONObject merged = getData(
                new MergeCaseDto(req.getCaseId(), req.getChooseContent(), EMPTY_STR, DEFAULT_ENV, 0L));
    ...
    public void editRecord(RecordUpdateReq req) {
        ...
        if (!record.getChooseContent().equals(req.getChooseContent())) {
            ...
            if (room != null && room.players.size() > 0) {
                Log.info("[异步编辑任务属性]入参={}, 这些人正在编辑={}", req.toString(), room.getRoomPlayersName());
                throw new CaseServerException("当前" + room.getRoomPlayersName() + "正在修改，不允许修改圈选用例",
    ...
    public ExecRecord wsClearRecord(RecordWsClearReq req) {
        ...
        JSONObject merged = getData(new MergeCaseDto(record.getCaseId(), record.getChooseContent(),
                record.getCaseContent(), record.getEnv(), record.getId()));
    ...
    private RecordListResp buildList(ExecRecord record) {
        ...
        JSONObject object = getData(new MergeCaseDto(record.getCaseId(), record.getChooseContent(),
                record.getCaseContent(), record.getEnv(), 0L));
        ...
        resp.setExpectEndTime(compareToOriginalDate(record.getExpectEndTime()) ? null : record.getExpectEndTime());
        return resp;
    }
    ...
    public JSONObject getData(MergeCaseDto dto) {
        ...
        if (dto.getRecordId() > 0L) {
            websocketCaseContent = WebSocket.getRoom(false, BitBaseUtil.mergeLong(dto.getRecordId(), dto.getCaseId()))
                    .getTestCaseContent();
        }
        ...
        if (!StringUtils.isEmpty(dto.getChooseContent()) && !dto.getChooseContent().contains(OE_PICK_ALL)) {
            ...
            // 给未来的环境选择做好打算...
            if (EnvEnum.TestQaEnv.getValue().equals(dto.getEnv())
                    || EnvEnum.TestRdEnv.getValue().equals(dto.getEnv())) {
                // 似乎是想用BFS做广度遍历
                JSONObject caseRoot = content.getJSONObject("root");
                Stack<JSONObject> objCheck = new Stack<>();
                Stack<IntCount> iCheck = new Stack<>();
