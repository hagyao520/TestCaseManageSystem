...
public class TreeUtil {
    ...
    /**
     * ******深度递归，获取每个用例的具体内容，读出所有的计数********** 根据一份测试用例，递归获取其中所有底部节点的用例执行情况 分为两种情况：
     * ①当前节点有子节点
     * <1>如果当前节点状态为1、5、9，那么值收集下游节点的个数total和，然后变成自己对应的状态个数+=childTotalSum,total++
     * <2>如果节点为4，则忽略 <3>如果节点为null，则total++ ②当前节点无子节点 计数体对应的状态数++,total++
     *
    ...
    public static void getPriority0(Stack<JSONObject> stackCheck, Stack<IntCount> iCheck, JSONObject parent) {
        ...
        for (; i.get() < children.size();) {
            ...
            i.add();
            if (obj.getJSONObject("data").containsKey("priority")
                    && obj.getJSONObject("data").getLong("priority") == 1L) {
                continue;
            } else {
                if (obj.getJSONArray("children").size() == 0) { // 当前是叶子结点
                    children.remove(obj);
    ...
    public static void exportDataToXml(JSONArray children, Element rootTopic) {
        ...
        for (Object o : children) {
            ...
            Element topic = topics.addElement("topic").addAttribute("id", dataObj.getString("id"))
                    .addAttribute("modified-by", "刘智").addAttribute("timestamp", dataObj.getString("created"));
            ...
            if (priority != null && !priority.equals("")) {
                ...
                marker_refs.addElement("marker-ref").addAttribute("marker-id", priority);
            }
