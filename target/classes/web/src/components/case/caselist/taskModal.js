...
class TaskModal extends React.Component {
  ...
  getOwnerList1 = () => {
    ...
    request(url, {
      method: 'GET',
      params: { productLineId: 1, caseType: 0 },
    }).then(res => {
      if (res.code === 200) {
        this.setState({
          ownerList1: res.data,
  ...
  render() {
    const { ownerList, userList, requirementSeach, fetching } = this.props;
    const { getFieldDecorator } = this.props.form;
    const { ownerList1, choiseDate, radioValue, selectValue, resource } = this.state;
    const radioStyle = {
      display: 'block',
      height: '80px',
      marginTop: '10px',
